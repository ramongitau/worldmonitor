...
import type { UcdpViolenceEvent } from '../../conflict/v1/service_server';
import type { UnrestEvent } from '../../unrest/v1/service_server';
import type { Earthquake } from '../../seismology/v1/service_server';
...
export interface IntelligenceServiceHandler {
  ...
  getCiiHistory(ctx: ServerContext, req: GetCiiHistoryRequest): Promise<GetCiiHistoryResponse>;
  getEventArchive(ctx: ServerContext, req: GetEventArchiveRequest): Promise<GetEventArchiveResponse>;
}
...
export function createIntelligenceServiceRoutes(
  ...
  return [
    ...
    {
      method: "GET",
      path: "/api/intelligence/v1/get-cii-history",
      handler: async (req: Request): Promise<Response> => {
        try {
          const pathParams: Record<string, string> = {};
          const url = new URL(req.url, "http://localhost");
          const params = url.searchParams;
          const body: GetCiiHistoryRequest = {
            region: params.get("region") ?? "",
          ...
          const result = await handler.getCiiHistory(ctx, body);
          return new Response(JSON.stringify(result as GetCiiHistoryResponse), {
            status: 200,
            headers: { "Content-Type": "application/json" },
          });
        } catch (err: unknown) {
    ...
    {
      method: "GET",
      path: "/api/intelligence/v1/get-event-archive",
      handler: async (req: Request): Promise<Response> => {
        try {
          const pathParams: Record<string, string> = {};
          const url = new URL(req.url, "http://localhost");
          const params = url.searchParams;
          const body: GetEventArchiveRequest = {
            startAt: Number(params.get("start_at") ?? "0"),
            endAt: Number(params.get("end_at") ?? "0"),
            countryCode: params.get("country_code") ?? "",
            eventTypes: params.getAll("event_types"),
          ...
          const result = await handler.getEventArchive(ctx, body);
          return new Response(JSON.stringify(result as GetEventArchiveResponse), {
            status: 200,
            headers: { "Content-Type": "application/json" },
          });
        } catch (err: unknown) {
